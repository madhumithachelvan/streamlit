    ...
    with col2:
        ...
        def display_example(example_text, scale_index, confidence_level, reasoning_text, font_size=""):
            ...
            st.radio(
                "Select a value:",
                scale_options,
                index = scale_index,# Link to session state (default to None)
            ...
            st.write(f"Selected value: {scale_options[scale_index]}")
...
def page6():
    ...
    with col2:
        ...
        # Retrieve current selection (could be None, string, or dict)
        current_style = st.session_state["responses"][current_index].get("style", {})

        # Handle different storage formats (backward compatibility)
        if isinstance(current_style, dict):
            current_numeric = current_style.get("numeric")
            current_label = current_style.get("label")
        elif isinstance(current_style, str):
            current_numeric = int(current_style.split(": ")[0]) if current_style else None
            current_label = current_style.split(": ")[1] if current_style else None
        else:
            current_numeric = None
            current_label = None

        # Reconstruct full label for radio display
        current_full_label = f"{current_numeric}: {current_label}" if current_numeric else None

        # Find index for radio widget
        index_value = scale_options.index(current_full_label) if current_full_label in scale_options else None

        def update_slider():
            st.session_state["responses"][current_index]["style"] = st.session_state["style_ratio"]

        # Display radio buttons
        selected_full_label = st.radio(
            "Select a value:",
            scale_options,
            index=index_value,
            key="style_ratio",
        ...
        if selected_full_label:
            numeric_value = int(selected_full_label.split(": ")[0])
            text_label = selected_full_label.split(": ")[1]

            st.session_state["responses"][current_index]["style"] = {
        ...
        # Retrieve current confidence data (handle both string and dict formats)
        current_confidence = st.session_state["responses"][current_index].get("confidence", {})

        # Handle existing data format (string or dictionary)
        if isinstance(current_confidence, dict):
            current_conf_num = current_confidence.get("numeric")
            current_conf_label = current_confidence.get("label")
            current_conf_full = f"{current_conf_num}: {current_conf_label}" if current_conf_num else None
        elif isinstance(current_confidence, str):
            if ": " in current_confidence:
                current_conf_num = int(current_confidence.split(": ")[0])
                current_conf_label = current_confidence.split(". ")[0].split(": ")[1] + "." + \
                                     current_confidence.split(". ")[1]
                current_conf_full = current_confidence
            else:
                current_conf_num = None
                current_conf_label = None
                current_conf_full = None
        else:
            current_conf_num = None
            current_conf_label = None
            current_conf_full = None

        # Find the index for selectbox
        confidence_index = confidence_options.index(
            current_conf_full) if current_conf_full in confidence_options else None

        # Display selectbox with formatted options
        ...
        # Store both numeric value and full text in session state
        if selected_confidence:
            # Split into numeric and label parts
            conf_parts = selected_confidence.split(". ", 1)  # Split on first period only
            numeric_value = int(conf_parts[0].split(": ")[0])
            label = f"{conf_parts[0].split(': ')[1]}. {conf_parts[1]}"

            st.session_state["responses"][current_index]["confidence"] = {
                "numeric": numeric_value,
                "label": label,
                "full": selected_confidence
            }
        else:
    ...
    with col2:
        ...
        if st.button("Back"):
            ...
            if user_id in st.session_state.get("submitted_users", set()):
                ...
                responses_df["style_score"] = responses_df['style'].str.get('numeric')
                responses_df["style_label"] = responses_df['style'].str.get('label')
                responses_df["confidence_score"] = responses_df['confidence'].str.get('numeric')
                responses_df["confidence_label"] = responses_df['confidence'].str.get('label')
