...
responses_dir = 'reannotations'

count = 0

file_list = []

file_list = glob.glob("survey_responses*.csv",root_dir=responses_dir)
file_list = [os.path.join(responses_dir,file) for file in file_list]

dataframes = [pd.read_csv(f) for f in file_list]
merged_df = pd.concat(dataframes, ignore_index=True)

# Write the merged DataFrame to a new CSV file
merge_name = os.path.join(responses_dir,f"merged_reannotations.csv")
